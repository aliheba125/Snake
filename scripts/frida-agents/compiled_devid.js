...
frida_java_bridge_default.perform(() => {
  try {
    const Settings = frida_java_bridge_default.use("android.provider.Settings$Secure");
    ...
    try {
      ...
      send("no FlutterSecureStorage");
    }
    const File2 = frida_java_bridge_default.use("java.io.File");
    const prefsDir = File2.$new(ctx.getFilesDir().getParent() + "/shared_prefs");
    const files = prefsDir.listFiles();
    if (files) {
      for (let i = 0; i < files.length; i++)
        send("file: " + files[i].getName());
    }
  } catch (e) {
    send("err: " + e);
  }
