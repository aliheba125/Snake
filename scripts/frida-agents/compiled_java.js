  ...
  frida_java_bridge_default.enumerateClassLoaders({
    onMatch(loader) {
      ...
      try {
        ...
        const known = [15799041, 15799553, 15799584, 15799585, 15865089, 15930625];
        ...
        for (const idx of explore) {
          try {
            const r = Native.ilil(idx);
            if (r && r.length > 0)
              send(`${T()} ilil(0x${idx.toString(16)}) = "${r.substring(0, 120)}"`);
          } catch (e) {
          }
        }
