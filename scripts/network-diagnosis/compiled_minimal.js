...
function hx(ptr2, len) {
  try {
    return Array.prototype.map.call(new Uint8Array(ptr2.readByteArray(len)), function(c) {
...
function txt(ptr2, len) {
  try {
    return ptr2.readUtf8String(Math.min(len, 4096));
  } catch (e) {
    return null;
  }
}
...
send({ t: "log", m: "agent_minimal ready; java=" + frida_java_bridge_default.available });
