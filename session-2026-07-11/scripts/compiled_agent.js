...
function txt(ptr2, len) {
  try {
    var s = ptr2.readUtf8String(Math.min(len, 2048));
...
function installNative() {
  ...
  if (!b) return false;
  try {
    Interceptor.attach(b.add(1447816), { onEnter: function(a) {
      emit({ t: "kdf", s1: "0x" + (a[1].toUInt32() >>> 0).toString(16), s2: a[2].toString() });
    } });
    Interceptor.attach(b.add(1442312), {
      onEnter: function(a) {
        this.o = a[2];
        var v = readVec(a[1]);
        emit({ t: "aes_in", len: v ? v.len : 0, hex: v ? hx(v.ptr, v.len) : null, txt: v ? txt(v.ptr, v.len) : null });
      },
      onLeave: function() {
        var v = readVec(this.o);
        emit({ t: "aes_out", len: v ? v.len : 0, hex: v ? hx(v.ptr, v.len) : null, txt: v ? txt(v.ptr, v.len) : null });
      }
    });
    emit({ t: "log", m: "native AES/KDF hooks @ " + b });
    return true;
  } catch (e) {
    emit({ t: "log", m: "native hook err " + e });
    return false;
  }
}
...
function hookNativeClass() {
  ...
  var N = null;
  try {
    N = frida_java_bridge_default.use(NAME);
  } catch (e) {
  }
  ...
  try {
    ...
    N.djp.implementation = function(x) {
      ...
      emit({ t: "djp", arg: x, len: r ? r.length : 0, hex: h });
      return r;
    };
    N.chl.implementation = function(bytes) {
      var r = this.chl(bytes);
      emit({ t: "chl", inlen: bytes ? bytes.length : 0, inhex: bytes ? b2h(bytes) : null, ret: r });
      return r;
    };
    try {
      N.aior.implementation = function(a, b) {
        emit({ t: "aior", a: "" + a, b: "" + b });
    ...
    try {
      N.awl.implementation = function(a) {
        emit({ t: "awl", a: "" + a });
        return this.awl(a);
      };
    } catch (e) {
...
emit({ t: "log", m: "agent init; java.available=" + frida_java_bridge_default.available });
ptraceBypass();
...
var master = setInterval(function() {
  ...
  if (!javaDone && frida_java_bridge_default.available) {
    try {
      frida_java_bridge_default.perform(function() {
        if (hookNativeClass()) javaDone = true;
      });
    } catch (e) {
    }
  }
  if (nativeDone && javaDone || ticks > 4e3) {
