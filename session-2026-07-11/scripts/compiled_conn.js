...
function b2h(arr) {
  try {
    var s = "";
    for (var i = 0; i < arr.length; i++) s += ("0" + ((arr[i] + 256) % 256).toString(16)).slice(-2);
    return s;
  } catch (e) {
    return null;
  }
...
function readVec(v) {
  try {
    ...
    if (l > 0 && l < 131072) return { ptr: b, len: l };
  } catch (e2) {
  }
  return null;
}
function hx(ptr2, len) {
  try {
    var ba = ptr2.readByteArray(len);
    return Array.prototype.map.call(new Uint8Array(ba), function(c) {
...
function txt(ptr2, len) {
  try {
    var s = ptr2.readUtf8String(Math.min(len, 4096));
    return s;
  } catch (e) {
  }
  return null;
...
function installNative() {
  ...
  if (!b) {
    send({ t: "log", m: "libengine NOT loaded" });
    return false;
  }
  Interceptor.attach(b.add(1447816), {
    onEnter: function(a) {
      lastKdf = { s1: "0x" + (a[1].toUInt32() >>> 0).toString(16), s2: a[2].toString() };
      send({ t: "kdf", s1: lastKdf.s1, s2: lastKdf.s2 });
    }
  });
  Interceptor.attach(b.add(1442312), {
    onEnter: function(a) {
      this.o = a[2];
      var v = readVec(a[1]);
      send({ t: "aes_in", kdf: lastKdf, len: v ? v.len : 0, hex: v ? hx(v.ptr, v.len) : null });
    },
    onLeave: function() {
      var v = readVec(this.o);
      send({ t: "aes_out", len: v ? v.len : 0, hex: v ? hx(v.ptr, v.len) : null, txt: v ? txt(v.ptr, v.len) : null });
    }
  });
  try {
    Interceptor.attach(b.add(1565e3), { onEnter: function() {
      send({ t: "call", fn: "FUN_0017e148" });
    } });
  } catch (e) {
  }
  try {
    Interceptor.attach(b.add(1611636), { onEnter: function() {
      send({ t: "call", fn: "FUN_00189774" });
    } });
  } catch (e) {
  }
...
function installSSL() {
  function expAny(n) {
    var f = null;
    try {
      f = Module.getGlobalExportByName(n);
    } catch (e) {
    }
    if (!f) {
  ...
  if (w) Interceptor.attach(w, { onEnter: function(a) {
    var n = a[2].toInt32();
    send({ t: "ssl_w", len: n, txt: txt(a[1], n) });
  } });
  if (r) Interceptor.attach(r, { onEnter: function(a) {
    this.b = a[1];
  }, onLeave: function(x) {
    var n = x.toInt32();
    if (n > 0) send({ t: "ssl_r", len: n, txt: txt(this.b, n) });
  } });
...
function getNativeClass() {
  ...
  try {
    return frida_java_bridge_default.use(NAME);
  } catch (e) {
  }
  try {
    ...
    for (var i = 0; i < loaders.length; i++) {
      try {
        var f = frida_java_bridge_default.ClassFactory.get(loaders[i]);
        var C = f.use(NAME);
        if (C) return C;
      } catch (e) {
      }
...
function installPMHooks() {
  try {
    var PM = frida_java_bridge_default.use("android.app.ApplicationPackageManager");
    ...
    send({ t: "log", m: "PM hook err " + e });
  }
  try {
    var RT = frida_java_bridge_default.use("java.lang.Runtime");
    RT.exec.overload("[Ljava.lang.String;").implementation = function(arr) {
      try {
        send({ t: "exec", cmd: arr.join(" ") });
      } catch (e) {
      }
...
function installJavaHooks() {
  ...
  try {
    ...
    N.djp.implementation = function(x) {
      ...
      var h = null;
      try {
        h = b2h(frida_java_bridge_default.array("byte", r));
      } catch (e) {
      }
      send({ t: "djp", arg: x, len: r ? r.length : 0, hex: h });
      return r;
    };
    N.chl.implementation = function(bytes) {
      var r = this.chl(bytes);
      send({ t: "chl", inlen: bytes ? bytes.length : 0, inhex: bytes ? b2h(bytes) : null, ret: r });
      return r;
    };
    send({ t: "log", m: "Java Native passive hooks installed" });
    return true;
  } catch (e) {
    send({ t: "log", m: "java hook err: " + e });
    return false;
  }
}
...
function mkVec(bytes) {
  ...
  vec.add(8).writePointer(buf.add(bytes.length));
  vec.add(16).writePointer(buf.add(bytes.length));
  return vec;
}
function mkEmptyVec() {
  var vec = Memory.alloc(24);
  vec.writePointer(ptr(0));
  vec.add(8).writePointer(ptr(0));
  vec.add(16).writePointer(ptr(0));
  return vec;
}
rpc.exports = {
  ...
  kdf: function(seed1, seed2) {
    var b = baseOf("libengine.so");
    if (!b) return null;
    var fn = new NativeFunction(b.add(1447816), "void", ["pointer", "uint32", "int64", "int64"]);
    var out = mkEmptyVec();
  ...
  aes: function(keyHex, inHex) {
    var b = baseOf("libengine.so");
    if (!b) return null;
    var fn = new NativeFunction(b.add(1442312), "long", ["pointer", "pointer", "pointer"]);
  ...
  kdfOnApp: function(seed1, seed2) {
    return new Promise(function(resolve) {
      ...
      var iv = setInterval(function() {
        waited += 30;
        if (state.result !== null || waited > 8e3) {
          clearInterval(iv);
          try {
            listener.detach();
          } catch (e) {
  ...
  init: function() {
    ...
    try {
      ...
      send({ t: "log", m: "installNative err " + e });
    }
    try {
      if (frida_java_bridge_default.available) frida_java_bridge_default.perform(function() {
        try {
          jav = installJavaHooks();
        } catch (e) {
          send({ t: "log", m: "javahook err " + e });
        }
      });
    } catch (e) {
      send({ t: "log", m: "java perform err " + e });
    }
    return { nativeHooks: nat, javaHooks: jav, javaAvailable: frida_java_bridge_default.available };
  },
  ...
  dump: function(start, end, callDjp) {
    ...
    if (!frida_java_bridge_default.available) {
      send({ t: "log", m: "Java not available for dump" });
      return out;
    }
    frida_java_bridge_default.perform(function() {
      var N = getNativeClass();
      if (!N) {
        send({ t: "log", m: "Native class not found for dump" });
        return;
      }
      for (var i = start; i < end; i++) {
        try {
          var s = N.ilil(i);
          if (s !== null && s !== void 0 && ("" + s).length > 0) out.ilil["0x" + i.toString(16)] = "" + s;
        } catch (e) {
        }
        if (callDjp) {
          try {
            var b = N.djp(i);
            if (b && b.length) out.djp["0x" + i.toString(16)] = b2h(frida_java_bridge_default.array("byte", b));
          } catch (e) {
          }
        }
      }
  ...
  dumpList: function(indices, callDjp) {
    var out = { ilil: {}, djp: {} };
    if (!frida_java_bridge_default.available) return out;
    frida_java_bridge_default.perform(function() {
      var N = getNativeClass();
      if (!N) {
        send({ t: "log", m: "Native class not found" });
        return;
      }
      indices.forEach(function(i) {
        try {
          var s = N.ilil(i);
          out.ilil["0x" + i.toString(16)] = s === null || s === void 0 ? null : "" + s;
        } catch (e) {
          out.ilil["0x" + i.toString(16)] = "ERR:" + e;
        }
        if (callDjp) {
          try {
            var b = N.djp(i);
            out.djp["0x" + i.toString(16)] = b && b.length ? b2h(frida_java_bridge_default.array("byte", b)) : null;
          } catch (e) {
          }
        }
      });
...
send({ t: "log", m: "agent_conn ready; java.available=" + frida_java_bridge_default.available });
