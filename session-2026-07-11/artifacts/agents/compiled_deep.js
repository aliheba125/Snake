  ...
  frida_java_bridge_default.enumerateClassLoaders({
    onMatch(loader) {
      ...
      try {
        ...
        let ilCount = 0;
        const groups = [241, 242, 243, 244, 245];
        const subgroups = [17, 18, 19, 20, 21, 22, 23, 24, 25, 33, 34, 49, 65];
              ...
              try {
                ...
                if (r && r.length > 0) {
                  ...
                  for (let j = 0; j < Math.min(r.length, 96); j++)
                    hex += ("0" + (r[j] & 255).toString(16)).slice(-2);
        ...
        try {
          Native.aior.overload().implementation = function() {
            send(`${T()} aior() CALLED`);
            return this.aior();
          };
          send(`${T()} [HOOK] aior OK`);
        ...
        try {
          Native.awl.overload().implementation = function() {
            send(`${T()} awl() CALLED`);
            return this.awl();
          };
          send(`${T()} [HOOK] awl OK`);
