...
4. **We did not prove any login bypass or account access.** The seller REST API (`rest.snakeseller.com`) uses separate email+password auth and rejects unauthenticated requests. Nothing here bypasses it.
...
- **Not proven**: that z is "used" downstream in any meaningful way; the meaning of the response; any ability to log in, access accounts, or bypass the authenticated REST API.
