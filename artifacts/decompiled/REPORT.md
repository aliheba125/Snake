# تقرير تحليل تطبيق SE_2.2.6.apk (Snake Engine)

> تم إنتاج هذا المجلد عبر تحليل لملف `SE_2.2.6.apk` الخاص بحساب المالك.
...
├── lib/arm64-v8a/
│   ├── libapp.so         → منطق التطبيق الحقيقي (كود Dart مترجم AOT)   ← تم تحليله بـ Blutter
...
## 4. الأدوات المستخدمة في التحليل

| الأداة | الإصدار | الغرض |
|--------|---------|-------|
| **jadx** | 1.5.1 | تحليل `classes.dex` إلى كود Java قابل للقراءة |
...
2. **`libengine.so` محمي:** مجرّد من الرموز (Stripped) ويستخدم تسجيل JNI ديناميكي (`JNI_OnLoad` فقط)، ويحمّل الرموز بشكل غير مباشر عبر `libdl`. تحليله الكامل يتطلب تحليل يدوي عميق (IDA/Ghidra + Frida).
